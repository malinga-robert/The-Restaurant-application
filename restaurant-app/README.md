...
## A Restaurant Management Application using (MongoDB-Express-React-Node)  
...
2.```cd backend```  
3. Create a .env file and enter your MongoDB Atlas connection string.  
Format is the following: ```MONGO_DB_URL=mongodb+srv://<username>:<password>@<server>/<databaseName>?retryWrites=true&w=majority```
4. ```npm install && npm start```  
5. Type localhost:5000 in your browser and check if you would see "Cannot GET /".  If you see this message then it means that you were able to run the backend server.  
